...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    WM_QUIT, WM_TIMER, WM_USER,
};
use windows_core::{GUID, PCWSTR, Result, implement};

use crate::devices::{Devices as State, Discovered, Hardware};

mod identity;

const MSG_SYNC: u32 = WM_APP + 1;
const MSG_VOLUME: u32 = WM_APP + 2;
const MSG_DEVICE: u32 = WM_APP + 3;
...
pub fn sync(thread: u32) {
    post(thread, MSG_SYNC, 0, 0);
}

pub fn stop(thread: u32) {
    post(thread, WM_QUIT, 0, 0);
}

fn post(thread: u32, message: u32, wparam: usize, lparam: isize) {
    if thread == 0 {
        return;
    }
    unsafe {
...
impl Audio {
    ...
    fn refresh(&mut self) {
        ...
        for device in &devices {
            let Some(endpoint) = endpoint_id(device) else { continue };

            // Adopting a mic before Windows can identify its hardware would file it under a
            // key that belongs to the port, so it is left out until the next pass.
            let Some(hardware) = identity::hardware(&endpoint) else {
                self.incomplete = true;
                continue;
            };

            if let Some(found) = self.bind(device, endpoint, hardware, default_endpoint.as_deref())
            {
                discovered.push(found);
            }
        }

        self.state.sync(discovered);
        self.enforce_all();
    }
    ...
    fn poll(&mut self) {
        if self.bindings.is_empty() || self.incomplete {
            self.refresh();
            return;
        }
