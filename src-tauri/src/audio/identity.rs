...
use crate::devices::Hardware;

// None means the device tree cannot place this endpoint yet, which happens while it is still arriving.
pub fn hardware(endpoint: &str) -> Option<Hardware> {
    let path = HSTRING::from(format!(r"SWD\MMDEVAPI\{endpoint}"));
    let mut node = 0u32;
    unsafe {
